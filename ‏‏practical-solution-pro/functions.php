...
function ps_enqueue_scripts() {
    // CSS الرئيسي - ملف واحد مدمج ومضغوط
    wp_enqueue_style(
        'ps-main-style',
    ...
    // JavaScript الرئيسي - ملف واحد مدمج ومضغوط
    wp_enqueue_script(
        'ps-main-script',
        PS_THEME_URI . '/dist/js/main.min.js',
        ['jquery'], // يعتمد على jQuery
        PS_THEME_VERSION,
        true // تحميل في الفوتر
    );
    
    // البيانات المحلية للجافا سكريبت
    wp_localize_script('ps-main-script', 'psAjax', [
        'ajaxurl' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('ps_nonce'), // استخدام Nonce واحد وثابت
    ...
    if (!is_admin()) {
        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', function() {
                navigator.serviceWorker.register('<?php echo esc_url(PS_THEME_URI . '/sw.js'); ?>')
...
// (يمكنك وضع هذا الكود بعد قسم "تحميل الملفات المحسن" مباشرة ليبقى الكود منظماً)
// =============================================================================

/**
 * تحميل ملفات CSS و JavaScript الخاصة بلوحة التحكم
 * Enqueue Admin Assets
 */
if (!function_exists('ps_enqueue_admin_assets')) {
    function ps_enqueue_admin_assets($hook) {
        // لا تقم بتحميل الملفات إلا في صفحات إعدادات القالب الخاصة بنا
        // للتحقق من أن $hook يحتوي على 'practical-solutions'
        if (strpos($hook, 'practical-solutions') === false) {
            return;
        }

        // تحميل ملف الأنماط الخاص بلوحة التحكم
        wp_enqueue_style(
            'ps-admin-styles', // اسم فريد للملف
            PS_THEME_URI . '/assets/admin/admin-styles.css', // المسار الصحيح للملف
            [], // لا يعتمد على أي ملفات أخرى
            PS_THEME_VERSION // رقم الإصدار لمنع مشاكل الكاش
        );

        // تحميل ملف السكريبت الخاص بلوحة التحكم
        wp_enqueue_script(
            'ps-admin-scripts', // اسم فريد للملف
            PS_THEME_URI . '/assets/admin/admin-scripts.js', // المسار الصحيح للملف
            ['jquery'], // يعتمد على مكتبة jQuery
            PS_THEME_VERSION, // رقم الإصدار
            true // تحميل السكريبت في تذييل الصفحة (footer)
        );

        // تمرير بيانات من PHP إلى JavaScript (مهم جداً لطلبات AJAX)
        wp_localize_script('ps-admin-scripts', 'psAdmin', [
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce'    => wp_create_nonce('ps_admin_nonce'), // مفتاح الأمان لطلبات AJAX
            'strings'  => [
...
// =============================================================================
// 22. إصلاح تعارض JQUERY (JQUERY CONFLICT FIX)
// =============================================================================
function ps_fix_jquery_conflict() {
    // التحقق من أننا في الواجهة الأمامية فقط وليس في لوحة التحكم
    if (!is_admin()) {
        // إلغاء تسجيل نسخة jQuery الافتراضية لووردبريس
        wp_deregister_script('jquery');
        
        // إعادة تسجيل نسخة jQuery الحديثة من ووردبريس
        // هذا يضمن عدم تحميل أي إضافات لنسخ قديمة
        wp_register_script('jquery', includes_url('/js/jquery/jquery.min.js'), false, null, true);
        
        // تحميل النسخة الصحيحة
        wp_enqueue_script('jquery');
    }
}
// استخدام أولوية عالية (رقم صغير) لضمان تنفيذ هذا الكود قبل أي إضافة أخرى
add_action('wp_enqueue_scripts', 'ps_fix_jquery_conflict', 1);
